...
def prices_chart(result_df: pd.DataFrame) -> go.Figure:
    x = result_df["datetime"].astype(str)
    ...
    fig = make_subplots(
        ...
        subplot_titles=["Ceny energií (EUR/MWh)", "Poptávka tepla (MWh)"],
    )

    fig.add_trace(go.Scatter(
        x=x, y=result_df["EE_price_EUR_MWh"],
        name="EE", line=dict(color=COLORS["accent"], width=2), mode="lines",
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=x, y=result_df["Gas_price_EUR_MWh"],
        name="Plyn", line=dict(color=COLORS["blue"], width=2), mode="lines",
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=x, y=result_df["Heat_price_EUR_MWh"],
...
def margin_heatmap(result_df: pd.DataFrame) -> go.Figure:
    ...
    z = result_df[valid_cols].T.values
    x = result_df["datetime"].astype(str).tolist()

    fig = go.Figure(go.Heatmap(
        z=z, x=x, y=valid_labels,
        colorscale=[
            [0.0, COLORS["red"]],
            [0.5, COLORS["surface2"]],
            [1.0, COLORS["green"]],
